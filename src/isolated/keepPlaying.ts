...
import { initialResumeState, reduceResume, type ResumeState } from './resume.ts'

let watched: HTMLVideoElement | null = null
let enabled = false

/**
 * The one decision, kept in src/isolated/resume.ts so it can be tested in node
 * against the iPhone leaving sequence rather than only on a phone.
 */
let resumeState: ResumeState = initialResumeState()

function onVisibility(): void {
  // The real value — the spoof is in the other world, this listener is ours.
  const { state } = reduceResume(resumeState, { type: document.hidden ? 'hidden' : 'visible' })
  resumeState = state
}

function onPlaying(): void {
  const { state } = reduceResume(resumeState, { type: 'playing', at: Date.now() })
  resumeState = state
}
...
function onPause(): void {
  ...
  // When we own the media-session handler, this is the immediate, certain answer;
  // the reducer below is what catches the case where the site has stolen it.
  if (pausedByUser()) {
    log('배경재생: 사용자가 멈춤 — 그대로 둔다')
    return
  }

  const decision = reduceResume(resumeState, { type: 'pause', at: Date.now() })
  resumeState = decision.state
  if (!decision.resume) {
    if (document.hidden) log('배경재생: 나간 뒤의 멈춤 — 사용자 것, 그대로 둔다')
    return
  }
...
export function enableKeepPlaying(): void {
  if (!enabled) {
    enabled = true
    resumeState = { ...initialResumeState(), hidden: document.hidden }

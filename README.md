...
진단의 **자막 선택** 줄 아래에 선택기가 무엇을 보고 그렇게 판단했는지가 같이 남습니다.

```
자막 선택: 영상이 이미 내 언어 (자막 안 켬)
  ↳ 내 언어 ko · 플레이어 트랙 1 · 응답 트랙 1 · 영상 언어 ko · 경로 :data
```

판정만으로는 답할 수가 없었습니다. `native-language` 는 한국어 영상에서 맞고 영어 영상에서 틀린데, **어느 쪽인지 알려면 그 영상을 따로 열어 재야 했습니다.** 보고 하나당 왕복 한 번이고, 도구가 있는 사람만 할 수 있는 일이었습니다. 이제 덤프 하나로 끝납니다.

`내 언어`(`want`)가 특히 중요합니다. 아래 모든 판단이 브라우저 언어 목록에서 나오는데, **덤프의 다른 어느 줄도 그 목록이 실제로 무엇이었는지 보여 주지 않습니다.**

`플레이어 트랙` 과 `응답 트랙` 을 나란히 두는 것도 이유가 있습니다. 둘 다 0이면 자막 없는 영상이고, 플레이어만 0이면 **모바일 플레이어가 목록을 안 채우는 그 벽**이라 폴백이 받아야 하는 상황입니다. 고치는 방향이 정반대입니다.

`경로`(`via`)는 `player` 면 플레이어 목록으로, `:data` 면 응답에서 주워 둔 것으로 판단했다는 뜻입니다.
...
## PiP 버튼은 시청 화면에만 붙습니다

검색 결과나 피드에는 붙이지 않습니다. **인라인 미리보기를 작은 화면으로 띄울 이유가 없고**, 미리보기가 깔린 목록은 스크롤로 계속 움직이는 화면이라 고정 버튼이 있을 자리가 아닙니다.

실기기에서 신고된 문제가 이것이었습니다. 검색 목록에서 버튼이 미리보기에 붙었고, `position: fixed` 인 채로 목록이 스크롤되자 **검색창 위와 유튜브 자기 음소거 버튼 위**에 앉았습니다. 증상은 "소리를 켤 수가 없다" 로 왔는데, 못 누른 게 아니라 **우리 버튼을 누르고 있었던 것**입니다.

원인은 두 군데였습니다.

**클램프.** 위치를 `Math.max(visibleTop + inset, top)` 으로 뷰포트 안에 밀어 넣고 있었습니다. 영상이 40px 만 보여도 버튼은 계속 뜨고, 클램프가 그걸 화면 맨 위로 끌어올립니다. 이제 클램프하지 않습니다. **버튼은 영상의 것이고, 영상 위에 온전히 앉지 못하면 안 보입니다.** 자기 영상에서 떨어져 나간 버튼은 없는 버튼보다 나쁩니다. 남의 컨트롤을 가리니까요.

**영상 고르기 점수에 "보이는가" 가 없었습니다.** 그래서 미리보기가 여럿인 페이지에서 화면 밖 영상이 이길 수 있었습니다. 신고가 "하나 재생될 때는 문제가 없다" 였던 이유가 그것입니다. 지금은 보이는지가 나머지 항목을 다 합한 것보다 무겁고, `tests/pip-placement.test.ts` 가 그 부등식을 지킵니다.

붙일 화면은 경로로 정합니다. `<video>` 개수로 세지 않습니다 — 시청 화면에도 예비 영상이 하나 더 있을 수 있고, 피드도 스크롤하다 보면 하나만 남을 수 있어서 양쪽 다 틀립니다. 쇼츠는 뺐습니다. **플레이어 옷을 입은 피드**입니다.
...
### `ITEM_NOT_UPDATABLE` 이 뜰 때

업로드가 이 오류로 거절되는 일이 자주 있습니다. 메시지는 세 가지 상태를 한꺼번에 가리킵니다.

```
The item cannot be updated now because it is in
pending review, ready to publish, or deleted status.
```

**어느 쪽인지 API 로는 알 수 없습니다.** `items.get` 은 업로드 상태만 돌려주고 심사·게시 상태는 안 내놓습니다. 대시보드를 열어 버튼이 무엇으로 보이는지가 유일한 판별법입니다.

| 대시보드 버튼 | 상태 | 할 일 |
|---|---|---|
| 게시 | 심사는 끝났고 게시가 안 됨 | 게시를 누르면 잠금이 풀린다 |
| 심사 취소 | 심사 중 | 기다리거나 취소한다 |
| **제출하여 검토받기** | **초안** | 아래를 읽는다 |

세 번째가 함정입니다. **심사를 취소한 뒤에도 그때 올라간 패키지가 초안에 남아 있고, 그 위로는 API 업로드가 막힙니다.** 대시보드 편집은 되는데 API 만 거절당하는 상태라, "취소했으니 이제 되겠지" 하고 워크플로를 돌리면 계속 실패합니다.

그때는 **대시보드 패키지 탭에서 zip 을 직접 올리는 것이 가장 빠릅니다.** 릴리스에 붙어 있는 `oc-ad-bye-pass-vX_Y_Z.zip` 을 끌어다 놓으면 됩니다.

**순서를 뒤집지 마십시오.** 초안에 얹혀 있는 패키지는 마지막으로 업로드에 *성공한* 버전이지, 마지막으로 태그를 단 버전이 아닙니다. 확인하지 않고 제출하면 몇 버전 전 코드가 심사를 받습니다. 어느 버전이 올라가 있는지는 릴리스 워크플로 로그에서 `Publish successful` 이 마지막으로 찍힌 태그를 보면 됩니다.

**게시 방법 설정도 한 번 확인해 두십시오.** "직접 게시" 로 돼 있으면 심사를 통과할 때마다 게시 대기 상태에서 멈추고, 그때마다 다음 업로드가 막힙니다.
...
감사가 실패하면 **이슈를 열고 저장소 소유자를 담당자로 지정합니다.** 이미 열려 있는 같은 제목의 이슈가 있으면 새로 만들지 않고 댓글만 답니다 — 매일 새 이슈가 쌓이면 아무도 안 봅니다.

담당자 지정이 핵심입니다. `github-actions[bot]` 이 연 이슈에 아무도 없으면 알림이 **저장소 지켜보기 설정에만** 의존하는데, 그건 언제든 꺼질 수 있고 켜져 있어도 다른 활동에 묻힙니다. 담당자로 지정하면 설정과 무관하게 알림이 가고 "나에게 할당됨" 목록에 남습니다. 그 목록은 짧아서 실제로 읽힙니다.

이름을 못 찾으면 담당자 없이 다시 시도합니다. `gh issue create` 는 해결 못 하는 담당자에 대해 통째로 실패하는데, **이름 하나 때문에 경보 자체를 잃는 것이 담당자 없는 경보보다 나쁩니다.**

### 언제 도나

두 시점입니다. **매일 정오**(크론 `0 3 * * *`, UTC 03:00)와 **릴리스 직후**입니다.

질문이 다릅니다. 크론은 "유튜브가 바뀌었나" 를 봅니다 — 코드는 그대로인데 어느 날 안 되는 그 상황이라, 시간이 트리거여야 합니다. 릴리스 직후는 "**우리가 방금 뭘 깨뜨렸나**" 를 봅니다. 크론만 있으면 그 답을 최대 24시간 기다려야 하고, 그동안 깨진 빌드가 스토어 심사를 지나 사용자에게 내려갑니다.

릴리스가 거는 감사는 `--ref` 가 **태그**입니다. main 으로 돌리면 릴리스 뒤에 들어온 커밋을 검사하게 되고 그건 나간 것이 아닙니다. 필터 봇이 하루 한 번 커밋하므로 실제로 어긋날 수 있습니다.

**더 자주 돌리는 것은 역효과입니다.** CI 러너는 데이터센터 IP 이고 로그인도 안 돼 있어 유튜브가 이미 봇으로 취급합니다. 재생 응답이 반쪽으로 오고 내용 검사가 관측불가로 내려가는데, 더 두드리면 그 판정이 강해질 뿐입니다. 관측불가가 늘면 감시가 아무것도 안 보게 됩니다. 감시 대상도 그렇게 빨리 안 바뀝니다 — 렌더러 이름이 갈아엎히는 건 몇 달에 한 번이지 시간 단위가 아닙니다.

### 기능이 조용히 죽는 것 자동 감시

위 감사는 **광고 누출**만 봅니다. 나머지 절반은 `scripts/audit-features.mjs` 가 봅니다.

차단은 결과로 잴 수 있습니다 — 광고가 보이는가. 나머지는 그럴 수가 없습니다. 자막 선택기는 플레이어의 `getOption('captions','tracklist')` 를 부르고, PiP 버튼은 `<video>` 에 붙고, 1계층은 `ytInitialPlayerResponse` 의 필드를 읽습니다. **유튜브가 그중 하나를 없애면 아무것도 안 터지고 광고도 안 새고, 기능만 조용히 멈춥니다.** 첫 보고는 "예전엔 됐는데" 라고 말하는 사용자입니다.

그래서 이쪽은 결과가 아니라 **코드가 기대는 계약**을 재고, `m.youtube.com` 에서도 잽니다. 모바일은 다른 플레이어이고 배포 주기도 다르며, 아이폰이 받는 쪽이 그쪽입니다.

**판정이 셋입니다.** `정상` · `깨짐` · `관측불가`. 세 번째가 이 감시를 쓸모 있게 유지합니다. 로그아웃 헤드리스 브라우저가 못 보는 것이 있습니다 — 2026-08-26 에 댓글 번역 버튼이 로그아웃 세션에는 아예 렌더링되지 않는다는 걸 알았고, 그걸 "깨짐" 이라고 불렀다면 틀린 경보였을 겁니다. **양치기 소년은 음소거당하고, 음소거된 감시는 없느니만 못합니다.** 그래서 관측하지 못한 것은 관측불가로 보고하고 런을 실패시키지 않습니다. 예전에 성립하던 계약이 지금 성립하지 **않는 것이 확인된** 경우에만 실패입니다.

**"유튜브가 안 내줬다" 와 "유튜브가 바뀌었다" 는 다릅니다.** CI 의 로그아웃 헤드리스 세션에는 유튜브가 축약된 응답을 줍니다 — `videoDetails` 도 `captions` 도 없이 네댓 개 필드뿐입니다. 첫 실전 실행이 그걸 "응답 구조 깨짐" 으로 신고했고, 정확히 이 감시가 피하려던 오경보였습니다. 이제 `videoDetails` 유무로 "진짜 재생 응답을 받았는가" 를 먼저 판정하고, 못 받았으면 내용 검사는 전부 관측불가로 내려갑니다. **봇으로 찍힌 것은 우리가 잃은 계약이 아닙니다.**

`watching(…)` 도 통과가 아닙니다. 거기서 영원히 멈추는 것이 자막 선택기의 실패 방식이고(폰이 하루 종일 대기 중에 얼어 있던 적이 있습니다), 우리가 봤을 때 아직 기다리는 중이면 그건 동작이 아니라 관측불가입니다.

**영상 사이트 밖도 봅니다.** 팝업 차단과 요소 선택기는 웹 전체에서 도는데 한동안 아무도 안 보고 있었습니다. 이 둘은 구조가 아니라 **동작**으로 잽니다 — 제스처 없이 `window.open` 을 불러 `null` 이 돌아오는지, 선택기 요청 키를 쓰면 오버레이가 붙는지. 코드 구조가 바뀌어도 유효하고, 사용자가 실제로 겪을 실패와 같은 것을 봅니다.

`example.com` 을 씁니다. 이런 용도로 유지되는 안정된 페이지이고 자기 광고가 없습니다. 우리 코드를 시험하려고 남의 실제 사이트를 매일 두드리는 것은 우리가 할 일이 아닙니다.

...
한 영상에 한 번만 적용하고 손을 떼는 것이 규칙이었고, 평범한 경우 하나에서 틀렸습니다. **CC 버튼을 누르는 것**입니다. 유튜브는 그 버튼에 자기가 저장해 둔 자막 상태로 답하고, 대부분 영어입니다. 가만히 두면 선택기가 완벽해 보이는데 버튼에 손이 가는 사람은 영어를 받고 이유를 알 방법이 없었습니다.

**CC 를 누르는 것은 언어를 고르는 행위가 아니라서** 되돌려 놓아도 사용자와 싸우는 게 아닙니다. 자막 메뉴를 열어 언어를 직접 고르는 것은 다릅니다. 그래서 되돌리기에 창과 횟수를 둡니다 — 적용 후 20초, 최대 3회. 그 뒤로 화면에 있는 것은 사용자의 것입니다.

되돌리기는 **플레이어가 다른 트랙을 실제로 보고할 때만** 합니다. 현재 트랙을 못 읽으면 "다르다" 가 아니라 "모르겠다" 이고, 모르는 채로 되돌리는 것이 확장이 사용자와 싸우기 시작하는 방법입니다. 판정은 `isSelectionApplied` 에 순수 함수로 있고 세 번째 상태(`null`)가 그 경계입니다.
...
- **댓글 번역과 더빙 음성 고정은 만들었다가 뺐습니다.** 둘 다 유튜브 플레이어나 UI 가 내주는 것에 얹는 기능이었고, 실측해 보니 그게 없을 때가 많았습니다. 댓글 번역은 로그인·비로그인 양쪽에서 번역 버튼이 아예 렌더링되지 않았고(실제 댓글 60개, 누를 수 있는 요소 480개 중 "번역" 이 붙은 것 0개), 음성 고정은 `getAvailableAudioTracks` 가 빌드마다 다릅니다. **켠 사람이 "안 되는 것" 과 "원래 없는 것" 을 구분할 수 없는 스위치는 없는 것만 못합니다.** 다시 만들 생각이라면 먼저 실기기에서 그 버튼이 실제로 존재하는지부터 재십시오.

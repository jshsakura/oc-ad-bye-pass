...
**같은 오류가 Orion 빌드에서도 난다면 대개 패키지 문제가 아니다.** Orion 의 설치기는
덮어쓰기를 못 하고, 지운 직후 첫 시도도 자주 실패한다. 순서를 이렇게 한다.

```
기존 확장 삭제 → + 로 설치를 시작해 설치 시트가 뜨면 닫기 → 다시 + 로 설치
```

같은 파일이 이 순서에서만 설치된다. 파일을 의심하기 전에 이걸 먼저 해본다 —
버전을 오르내리며 zip 을 비교해봐야 답이 안 나온다. 실제로 두 번 그렇게 샜다.

그래도 계속 실패하면 Orion 쪽 알려진 원인 둘을 걷어낸다. 파일 선택 실패의 공개된
